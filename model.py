...
class FastTransformer(Transformer):
    ...
    def ngram_cost(self, args, iters, targets, target_mask, out=None, iter_=0, return_logits=False):

        targets, out = prepare_cost(targets, out, target_mask.byte())
        logits = self.decoder[iter_].out(out)        
        if args.joint is True:
            word_loss = F.cross_entropy(logits,targets)
        probs = self.softmax(logits)
        del logits
        target_lens = torch.sum(target_mask, dim = -1).long().tolist()
        batch_size = len(target_lens)
        targets = targets.data.tolist()
        l_targets = len(targets)
        targets = self.shape(targets, target_lens)

        matchs = []
        if args.n == 1:
            gram_match = self.onegram_match(targets, target_lens, probs)
        elif args.n == 2:
            gram_match = self.twogram_match(targets, target_lens, probs)
        elif args.n == 3:
            gram_match = self.threegram_match(targets, target_lens, probs)
        elif args.n == 4:
            gram_match = self.fourgram_match(targets, target_lens, probs)
        for i in range(batch_size):
            matchs.append(gram_match[i][0]/gram_match[i][1])

        gram_loss = -1 * sum(matchs).div(batch_size)
        if args.joint is True:
            if iters < 30000:
                loss = loss = args.alph*3 * word_loss + (1-args.alph * 3) * gram_loss
            else:
                loss = args.alph * word_loss + (1-args.alph) * gram_loss
        else:
            loss = gram_loss
        return loss

    def onegram_match(self, targets, target_lens, probs):
        ...
        for i in range(batch_size):
            ...
            one_grams = Counter()
            for j in range(len(curr_tar)):
                one_grams[(curr_tar[j],0)] += 1

            gram_1 = []
            gram_count = []
            for one_gram in one_grams:
                gram_1.append(int(one_gram[0]))
                gram_count.append(one_grams[one_gram])

            match_gram = probs[begin:end, gram_1]
            ...
            batch_match.append((match_gram, target_lens[i]))
    ...
    def threegram_match(self, targets, target_lens, probs):
    ...
    def fourgram_match(self, targets, target_lens, probs):
