...
def run_fast_transformer(decoder_inputs, decoder_masks,\
    ...
    trg_unidx = model.output_decoding( ('trg', targets),unbpe=False)
    src_unidx = model.output_decoding( ('src', sources),unbpe=False)
    batch_size, src_len, hsize = encoding[0].size()
    #s = open("decoding/dec_source","a")
    #r = open("decoding/dec_ref","a")
    #l = len(src_unidx)
    #for i in range(l):
    #    s.write(src_unidx[i]+'\n')
    #    r.write(trg_unidx[i]+'\n')
    ...
    while True:
        ...
        #out_unidx = model.output_decoding( ('trg', decoding ),unbpe=False )
        #o = open("decoding/decode_out" + str(iter_),"a")
        #l = len(src_unidx)
        #for i in range(l):
        #    o.write(out_unidx[i]+'\n')
...
def decode_model(args, model, dev, evaluate=True, trg_len_dic=None,
    ...
    for iters, dev_batch in enumerate(dev):
        ...
        if args.dataset != "mscoco":
            ...
            targets, target_masks,\
            _, source_masks,\
            encoding, batch_size, rest = model.quick_prepare_mscoco(dev_batch, all_captions=all_captions, fast=(type(model) is FastTransformer), inputs_dec=args.inputs_dec, trg_len_option=args.trg_len_option, max_len=args.max_len, trg_len_dic=trg_len_dic, bp=args.bp, gpu=args.gpu)
            sources = None
